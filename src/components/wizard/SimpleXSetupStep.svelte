<script>
    ...
    // Briefing 045a: the wizard-internal orphan check (formerly here)
    // moved to App.svelte's top-level startup gate. By the time this
    // step renders, the App-level invariant guarantees there are no
    // orphaned contacts in the sidecar DB - the OrphanedContactsModal
    // either fired before the wizard mounted, or the DB was already
    // clean. Defense-in-depth here would only mask a regression in
    // App.svelte, not catch a real bug.
